...
void Vehicle::DrawModel(const DirectX::SimpleMath::Matrix aView, const DirectX::SimpleMath::Matrix aProj)
{
    ...
    m_heliModel.tailBoomLowerUpperEndCapShape->Draw(m_heliModel.tailBoomLowerEndCapMatrix, aView, aProj, m_heliModel.bodyColor);
...
void Vehicle::InitializeModel(Microsoft::WRL::ComPtr<ID3D11DeviceContext1> aContext, HeliData& aHeliData)
{
    ...
    */
    ...
    const float wingJetRearHousingHeight = wingJetCenterSize.x * 0.8f;
    ...
    const float wingJetRearHousingOffsetX = -1.7f;
    const float wingJetRearHousingOffsetY = 0.1f;
    ...
    */
    ...
    m_heliModel.mainRotorArmEndCapShape = DirectX::GeometricPrimitive::CreateSphere(aContext.Get(),  mainArmHeight);
    ...
    m_heliModel.tailRotorBladeEndCapShape = DirectX::GeometricPrimitive::CreateCylinder(aContext.Get(), tailBladeWidth, tailArmHeight);
    ...
    m_heliModel.tailRotorArmEndCapShape = DirectX::GeometricPrimitive::CreateSphere(aContext.Get(), tailArmHeight);
    ...
    m_heliModel.tailRotorBladeEdgeEndCapShape = DirectX::GeometricPrimitive::CreateCone(aContext.Get(), tailArmHeight, tailRotorLeadingEdgeDiameter);
    ...
    m_heliModel.tailRotorBladeStripe2Shape = DirectX::GeometricPrimitive::CreateBox(aContext.Get(), DirectX::SimpleMath::Vector3(tailBladeStripe1Width + rotorStripeOffset, tailBladeStripe2Length, tailArmHeight + rotorStripeOffset));
...
void Vehicle::RightHandSide(struct HeliData* aHeli, Motion* aQ, Motion* aDeltaQ, double aTimeDelta, float aQScale, Motion* aDQ)
{
    ...
    /*
    DebugPushUILineDecimalNumber("newQ.velocity     : ", newQ.velocity.Length(), "");
    DebugPushUILineDecimalNumber("aQ->velocity      : ", aQ->velocity.Length(), "");
    DebugPushUILineDecimalNumber("aDeltaQ->velocity : ", aDeltaQ->velocity.Length(), "");
    */
    ...
    //DebugPushUILineDecimalNumber("rotorForce.Length() : ", rotorForce.Length(), "");
    ...
    {
        if (rotorForce.y > -m_heli.gravity.y)
        {
            terrainNormalForce = DirectX::SimpleMath::Vector3::Zero;
            velocityUpdate += (m_heli.gravity) * static_cast<float>(aTimeDelta);
            velocityUpdate += rotorForce * static_cast<float>(aTimeDelta);
...
void Vehicle::RungeKutta4(struct HeliData* aHeli, double aTimeDelta)
{
    ...
    //DirectX::SimpleMath::Vector3 velocityUpdate = q.velocity + (dq1.velocity + 2.0 * dq2.velocity + 2.0 * dq3.velocity + dq4.velocity) / numEqns;
    ...
    /*
    DebugPushUILineDecimalNumber("dq1.velocity   : ", dq1.velocity.Length(), "");
    DebugPushUILineDecimalNumber("dq2.velocity   : ", dq2.velocity.Length(), "");
    DebugPushUILineDecimalNumber("dq3.velocity   : ", dq3.velocity.Length(), "");
    DebugPushUILineDecimalNumber("dq4.velocity   : ", dq4.velocity.Length(), "");
    DebugPushUILineDecimalNumber("velocityUpdate : ", velocityUpdate.Length(), "");
    */
    ...
    if (q.position.y + posUpdate.y <= aHeli->terrainHightAtPos - aHeli->localLandingGearPos.y)
    {
        if (q.velocity.y + velocityUpdate.y <= aHeli->gravity.y * aTimeDelta)
        {
            ...
            DebugPushUILineDecimalNumber("q.velocity.y     : ", q.velocity.y, "");
            DebugPushUILineDecimalNumber("aHeli->gravity.y * aTimeDelta : ", aHeli->gravity.y * aTimeDelta, "");
            */
            velocityUpdate = DirectX::SimpleMath::Vector3::Zero;
            q.velocity = DirectX::SimpleMath::Vector3::Zero;
...
void Vehicle::UpdateBladeLiftForce(const float aTimeStep)
{
    ...
    DebugPushUILineDecimalNumber("lift Time Mod  : ", lift / aTimeStep, "");

    lift = .5f * rho * omega2 * omega2 * Cl * y;
    DebugPushUILineDecimalNumber("lift Time Mod2 : ", lift, "");
    DebugPushUILineDecimalNumber("total lift     : ", m_heli.mainRotor.bladeVec[0].liftForcePerSecond + m_heli.mainRotor.bladeVec[1].liftForcePerSecond, "");
...
Utility::Torque Vehicle::UpdateBodyTorqueTestRunge(Utility::Torque aPendulumTorque, const float aTimeStep)
{
    ...
    */
    ...
    DebugPushTestLine(m_heli.centerOfMass, tailTorque.axis, 10.0f, 5.0f, DirectX::SimpleMath::Vector4(1.0f, 1.0f, 0.0f, 1.0f));
...
void Vehicle::UpdateVehicle(const double aTimeDelta)
{
    ...
    DebugPushTestLine(m_heli.q.position, m_heli.testAccelVec, 12.0f, 0.0f, DirectX::SimpleMath::Vector4(1.0f, 0.0f, 0.0f, 1.0f));
    
    UpdateAlignmentTorqueTest();
    UpdateAlignmentCamera();

    //DebugPushTestLinePositionIndicator(m_heli.q.position, 4.0f, 0.0f, DirectX::SimpleMath::Vector4(0.0f, 0.0f, 1.0f, 1.0f));

    DirectX::SimpleMath::Vector3 testAlignmentLine = m_heli.forward;
    testAlignmentLine = DirectX::SimpleMath::Vector3::Transform(testAlignmentLine, m_heli.alignment);
    DebugPushTestLine(m_heli.q.position, testAlignmentLine, 12.0f, 1.0f, DirectX::SimpleMath::Vector4(1.0f, 0.0f, 1.0f, 1.0f));

    DirectX::SimpleMath::Vector3 testAlignmenCamera = DirectX::SimpleMath::Vector3::UnitX;
    testAlignmenCamera = DirectX::SimpleMath::Vector3::Transform(testAlignmenCamera, m_heli.cameraOrientation);
    DebugPushTestLine(m_heli.q.position, testAlignmenCamera, 12.0f, 0.0f, DirectX::SimpleMath::Vector4(1.0f, 1.0f, 1.0f, 1.0f));
    
    DebugPushUILineDecimalNumber("m_heli.q.position.x : ", m_heli.q.position.x, "");
    DebugPushUILineDecimalNumber("m_heli.q.position.y : ", m_heli.q.position.y, "");
    DebugPushUILineDecimalNumber("m_heli.q.position.z : ", m_heli.q.position.z, "");
    DebugPushUILineDecimalNumber("m_heli.q.velocity.y : ", m_heli.q.velocity.y, "");
